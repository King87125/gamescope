...
fetch('updated_games_data.json') 
    ...
    .then(data => {
        gameData = data.map(game => ({
            ...
            price: game["Discounted Price"] || "N/A",
            onlinePlayers: [
...
function updateLineChart(games) {
    ...
    // 更新图表配置
    lineChart.setOption({
        title: { text: "每月在线峰值人数总和", left: "center" },
        grid: {
            left: "19%",   // 增加左侧空间
            top: "15%",
            bottom: "15%"
...
games.sort((a, b) => b.positiveReviews - a.positiveReviews);
    ...
    for (let i = 0; i < 5 && i < games.length; i++) {
        ...
        <span class="discount-price">￥${item.price}</span> <!-- 随机折扣价 -->
    `;
   ...
   div.addEventListener('click', () => {
    ...
    // 启动自动滚动
    startAutoScroll(rankingData, 5, 3000); // 每3秒滚动一次
}
